...
- 기본 설정은 `한국어 / 마스터 볼륨 100% / 보통`이다.
- `↑`, `↓`, `W`, `S`: 언어, 마스터 볼륨과 난이도 항목 이동
- `←`, `→`, `A`, `D`: 선택한 항목의 값 변경
- `Enter` 또는 `Space`: 설정을 적용하고 게임 시작
- `Esc`: 타이틀로 돌아가기
- 언어는 화면 문구와 검 이름에만 영향을 준다.
- 마스터 볼륨은 0~100% 범위를 10% 단위로 조절하며 배경음악과 효과음에 즉시 함께 적용한다.
- 난이도는 강화 비용, 자연 냉각 속도, 제한 시간과 실패 페널티 시작 단계에 영향을 주며 기본 성공 확률을 직접 변경하지 않는다.

| 난이도 | 강화 비용 | 자연 냉각 | 제한 시간 | 실패 페널티 시작 | 의도 |
| --- | ---: | ---: | ---: | ---: | --- |
| 쉬움 | 기본의 90% | 기본의 85% | 14초 | +10 이상 | 온도를 여유 있게 유지하며 조작을 익힌다. |
| 보통 | 기본의 100% | 기본의 100% | 12초 | +9 이상 | 원래 설계된 기준 밸런스를 사용한다. |
| 어려움 | 기본의 115% | 기본의 125% | 10초 | +4 이상 | 빠른 냉각과 짧은 시간으로 온도·타격 관리를 어렵게 한다. |
...
- `Esc`로 후퇴할 수 있지만 전투 패배와 동일하게 강화 단계가 1 감소한다.
- 플레이어 체력이 먼저 0이 되면 즉시 공격 입력을 잠그고 강화 단계가 1 감소한다.
- 전투 패배 시 골드와 기억 조각은 잃지 않는다.
- 승리하면 골드 중심 보상과 기억 조각 중심 보상 중 하나를 선택하고 다음 강화 구간이 열린다.
- `←`/`A`와 `→`/`D`로 보상을 선택하고 `Enter` 또는 `Space`로 확정한다.

| 구간 | 보스 | 공격 시퀀스 | 골드 선택 | 기억 선택 |
| --- | --- | --- | --- | --- |
| +4 | 잿불의 문지기 | 1.35초 예고 뒤 피해 24의 강한 단타 | 900 G | 300 G, 조각 2개 |
| +8 | 폭풍의 파수꾼 | 피해 8→8→10, 간격이 빨라지는 삼연격 | 1,500 G | 600 G, 조각 3개 |
| +12 | 기억을 삼키는 자 | 뒤틀린 공격→보랏빛 가짜 예고→빠른 진짜 공격 | 2,600 G | 1,000 G, 조각 4개 |

보랏빛 예고는 피해가 없는 잔상이며 방어하지 않는 것이 정답이다.
가짜 예고 직후에는 짧은 간격의 진짜 공격이 이어지므로 색상과 안내 문구를 읽어야 한다.
...
- 온도 게이지는 64~72도의 최적 공명 구간을 청록색으로 표시하며 중심값 68도에서 가장 높은 온도 점수를 얻는다.
...
- 실패 페널티 시작 단계는 쉬움 +10, 보통 +9, 어려움 +4다.
- 실패 페널티 시작 단계 미만에서 실패하면 강화 단계를 유지한다.
- 실패 페널티 시작 단계 이상에서는 기억 조각을 먼저 소모하고 단계를 유지한다.
- 기억 조각은 처음 2개를 보유하며 보스 처치 보상으로 추가 획득한다.
- 기억 조각이 없는 상태로 실패 페널티 시작 단계 이상에서 실패하면 강화 단계가 하나 내려간다.
...
- 강화 비용, 기본 확률과 실패 페널티는 `ForgeRules` 한 곳에서 관리한다.
- 난이도별 비용 배율과 실패 페널티 시작 단계는 `ForgeRules`, 자연 냉각 배율과 제한 시간은 `ForgeSession`에서 적용한다.
- 강화 구간 상한, 보스 도전과 엔딩 조건은 `ProgressionRules`에서 관리한다.
- 보스 수치, 공격 시퀀스와 공격·방어·반격 피해 계산은 `BattleRules`에서 관리한다.
- 공격 예고, 방어 판정과 실시간 시퀀스 상태는 `BattleSession`에서 관리한다.
- 승리 보상과 패배·후퇴 페널티의 일회성 반영은 `BattleSettlement`에서 관리한다.
...
- 우측 하단 채팅 박스는 모든 장면에서 유지되며 `T`로 입력을 시작한다.
- 메시지 입력 중에는 게임 시간과 장면 입력을 멈추고 `Enter`로 등록하거나 `Esc`로 취소한다.
- 확정된 메시지는 최신 줄이 아래에 오도록 쌓고 오래된 줄은 위로 밀며, 박스에 보이는 최근 6줄만 보관한다.
- 게임 화면 아래의 가상 키보드는 `WASD`, 방향키, `Enter`, `Space`, `Q`, `Esc` 입력을 표시한다.
- 새 입력은 약 0.065초간 노란색으로 점멸하고, 계속 누르는 입력은 청록색으로 구분한다.
